...
wt = (
    '''                                                
  Security Team [[re]][nstevanoski][[ye]]
  github.com/nstevanoski  
    '''
)
COLORS = {
    "re": "\u001b[31;1m",
...
if os.path.isfile('nstevanoski_log.txt'):
    with open('nstevanoski_log.txt', 'r') as r:
        data = r.readlines()
    api_id = data[0]
    api_hash = data[1]
    ...
    with open('nstevanoski_log.txt', 'w') as a:
        a.write(api_id + '\n' + api_hash)

client = TelegramClient('nstevanoski', api_id, api_hash)
...
async def main():
    async def getmem():
        ...
        with open('nstevanoski_members.txt', 'r') as r:
            users = json.load(r)
        count = 1
        i = 0
        for user in users:
    ...
    with open('nstevanoski_members.txt', 'w') as w:
